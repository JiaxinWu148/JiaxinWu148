...
public class MessageService {
    ...
    public int sendMessage(int userId, int roomId, int msgId) {
        ...
        if (msg.detectHateSpeech()) {
            if (chatRoom.getWarnedUsers().contains(userId)) {
                chatRoom.blockUser(userId);
                return -2;
            } else {
                chatRoom.addWarnedUsers(userId);
                return -1;
            }
...
//            String name = sender.getUsername();
            ...
            return msg.getId();
        } else if ((chatRoom.checkUser(userId) && (!chatRoom.checkBlock(userId)))) {
//            String name = sender.getUsername();
            User receiver = UserStore.make().getUser(msg.getReceiverId());
